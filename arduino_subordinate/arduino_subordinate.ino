...
void stepper_square_wave(uint8_t mode, uint8_t delay)
{ // delay in us
  if (mode == XY_AXIS)
  {
...
void motor_move_line(int8_t x_dir, int8_t y_dir, uint16_t dist, uint8_t delay)
{                    // (x, y) -> [-1, 1], dist in mm, both x, y, cannot be 0
...
void motor_move(int16_t x, int16_t y, uint8_t delay, uint8_t taxicab)
{ // in mm
...
void motor_move_piece(int16_t x_0, int16_t y_0, int16_t x_1, int16_t y_1, int8_t taxicab)
{ // in squares, but converted to mm
  if (x_0 < 0)
  {
    x_0 *= MM_PER_SQUARE;
    x_0 -= GRAVEYARD_GAP;
  ...
  Serial.println(x_0);
  Serial.println(y_0);
  motor_move(x_0, y_0, FAST_STEP_DELAY, false);
  delay(MOVE_DELAY);
